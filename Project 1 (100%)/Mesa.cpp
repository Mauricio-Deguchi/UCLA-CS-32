...
Mesa::Mesa(int nRows, int nCols)
    : m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nGarks(0), m_history(m_rows,m_cols)
{
...
bool Mesa::attackGarkAt(int r, int c, int dir)
{
    ...
    if (k < m_nGarks)    // gark is not dead
    {
        m_history.record(r, c);     // so let's record its position
    }
...
{
    History& trash = m_history;
    return trash;               // returns pointer to the Mesa's history object
}
